...
def create_environment(ctx: Context) -> None:
    ...
    ctx.run(
        f"conda create --name {PROJECT_NAME} python={PYTHON_VERSION} "
        "pip --no-default-packages --yes",
        echo=True,
        pty=not WINDOWS,
    )
...
def docker_build(ctx: Context, progress: str = "plain") -> None:
    ...
    ctx.run(
        f"docker build -t api:latest . "
        f"-f dockerfiles/api.dockerfile "
        f"--progress={progress}",
        echo=True,
        pty=not WINDOWS,
    )

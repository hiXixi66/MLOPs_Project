...
def test_dataset_size(train_dataset, val_dataset, test_dataset):
    ...
    """
    assert len(train_dataset) == int(0.7 * 75_000), "Training dataset size is incorrect."
    assert len(val_dataset) == int(0.15 * 75_000), "Validation dataset size is incorrect."
    assert (
...
def test_normalization(train_dataset, val_dataset, test_dataset):
    ...
    for dataset in [train_dataset, val_dataset, test_dataset]:
        for img, _ in dataset:
            assert img.max() <= 3, "Image is not normalized correctly (max > 3)."
            assert img.min() >= -3, "Image is not normalized correctly (min < -3)."
...
def test_dimensions(train_dataset, val_dataset, test_dataset):
    ...
    assert train_dataset[0][0].shape == expected_shape, "Training dataset image shape is incorrect."
    assert val_dataset[0][0].shape == expected_shape, "Validation dataset image shape is incorrect."
    assert test_dataset[0][0].shape == expected_shape, "Test dataset image shape is incorrect."
...
def test_no_corrupt_data(train_dataset):
    ...
    for img, _ in train_dataset:
        assert img is not None, "Corrupt or missing image detected in training dataset."
